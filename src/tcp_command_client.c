...
static PTC_ErrCode
tcpCommandClientSendCmdWithoutSecurity(TcpCommandClient *client,
                                       TC_Command *cmd) {
  if (!client && !cmd) {
...
PTC_ErrCode TcpCommandGetLidarCalibration(const void *handle, char **buffer,
                                          unsigned int *len) {
  return TcpCommandGet(handle, PTC_COMMAND_GET_LIDAR_CALIBRATION, (unsigned char**)buffer, len);
}

PTC_ErrCode TcpCommandGetLidarChannelConfig(const void *handle, char **buffer,
                                            unsigned int *len) {
  return TcpCommandGet(handle, PTC_COMMAND_GET_LIDAR_CHANNEL_CONFIG, (unsigned char**)buffer,
                       len);
}

PTC_ErrCode TcpCommandGetLidarFiretime(const void *handle, char **buffer,
                                       unsigned int *len) {
  return TcpCommandGet(handle, PTC_COMMAND_GET_LIDAR_FIRETIMES, (unsigned char**)buffer, len);
}
...
PTC_ErrCode TcpCommandGet(const void *handle, PTC_COMMAND command,
  ...
  PTC_ErrCode errorCode =
      tcpCommandClient_RecieveCmd(client, &cmd, (char**)buffer, len);
  return errorCode;
}
...
SSL_CTX *initial_client_ssl(const char *cert, const char *private_key,
  ...
  if (cert && private_key) {
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx, private_key, SSL_FILETYPE_PEM) == 0) {
      printf("%s:%d, load private key file failed,please check private key "
             "file path\n",
             __func__, __LINE__);
      return NULL;
    }
